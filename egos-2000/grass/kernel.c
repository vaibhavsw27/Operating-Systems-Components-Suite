...
static void intr_entry(uint id) {
    ...
    {
        proc_set[curr_proc_idx].no_interrupts+=1;
    }
...
static void proc_yield() 
{
    ...
    if (curr_proc_idx>0 && curr_proc_idx<=MAX_NPROCESS && curr_status!=PROC_UNUSED)
    {
        if (proc_set[curr_proc_idx].time_last_scheduled>0)
        {
            ulonglong runtime=current_time-proc_set[curr_proc_idx].time_last_scheduled;
    ...
    if (next_idx < MAX_NPROCESS) 
    {
        ...
        proc_set[next_idx].time_last_scheduled=current_time;//recording time for next process

        if (proc_set[next_idx].time_first_scheduled==0) //first time scheduling
        {
            proc_set[next_idx].time_first_scheduled=current_time;
        }
